...
export function runDocumentIndexContractTests(makeIndex) {
    describe('DocumentIndex contract', () => {
        ...
        describe('Edge Cases', () => {
            ...
            it('returns a structured error, not a crash, when fetch is called with an id that never existed', async () => {
                const index = makeIndex(new FakeEmbedder());
                await expect(index.fetch('00000000-0000-0000-0000-000000000000')).rejects.toBeInstanceOf(NotFoundError);
            });
        });
    });
}
